import Parser from "rss-parser";
import db from "./db";
import { processRawArticleForConfig } from "./aiService";

const parser = new Parser({
  customFields: {
    item: [
      ['media:content', 'mediaContent'],
      ['enclosure', 'enclosure'],
      ['image', 'image']
    ]
  }
});
let isSyncing = false;

export async function syncRSSNews() {
  if (isSyncing) return;
  isSyncing = true;
  try {
    const feeds = [
      { url: "https://thegrio.com/feed/", category: "diaspora", source_name: "The Grio" },
      { url: "https://www.blackenterprise.com/feed/", category: "finance", source_name: "Black Enterprise" },
      { url: "https://www.essence.com/feed/", category: "culture", source_name: "Essence" },
      { url: "https://www.theroot.com/rss", category: "diaspora", source_name: "The Root" },
      { url: "https://afro.com/feed/", category: "diaspora", source_name: "AFRO News" },
      { url: "https://www.jamaicaobserver.com/feed/", category: "caribbean", source_name: "Jamaica Observer" },
      { url: "https://allafrica.com/tools/headlines/rdf/latest/headlines.rdf", category: "africa", source_name: "AllAfrica" },
      { url: "https://www.africanews.com/feed/", category: "africa", source_name: "Africa News" },
      { url: "https://www.premiumtimesng.com/feed", category: "africa", source_name: "Premium Times Nigeria" },
      { url: "https://www.aljazeera.com/xml/rss/all.xml", category: "global", source_name: "Al Jazeera" },
      { url: "https://www.france24.com/en/rss", category: "global", source_name: "France 24" },
      { url: "https://www.democracynow.org/democracynow.rss", category: "global", source_name: "Democracy Now!" },
      { url: "https://theintercept.com/feed/?lang=en", category: "global", source_name: "The Intercept" },
      { url: "https://feeds.propublica.org/propublica/main", category: "global", source_name: "ProPublica" }
    ];
    
    for (const feed of feeds) {
      try {
        const parsed = await parser.parseURL(feed.url);
        for (const item of parsed.items.slice(0, 4)) {
           const urlHash = item.link || item.guid || Math.random().toString();
           const textDump = `${item.title}\n\n${item.contentSnippet || item.content || ""}`;
           
           let imageUrl = null;
           if (item.enclosure && item.enclosure.url) {
             imageUrl = item.enclosure.url;
           } else if (item.mediaContent && item.mediaContent['$'] && item.mediaContent['$'].url) {
             imageUrl = item.mediaContent['$'].url;
           } else if (item.image) {
             imageUrl = typeof item.image === 'string' ? item.image : item.image.url;
           }
           
           db.prepare('INSERT OR IGNORE INTO articles (url_hash, category, source_name, original_title, original_url, image_url, original_text_dump) VALUES (?, ?, ?, ?, ?, ?, ?)').run(
             urlHash, feed.category, feed.source_name, item.title || "Untitled", item.link || "#", imageUrl, textDump
           );
        }
      } catch (err) {
        console.error("Failed to parse feed", feed.url, err);
      }
    }

    const activeConfigs = db.prepare('SELECT DISTINCT reading_mode, lens_intensity FROM user_settings').all() as { reading_mode: string, lens_intensity: string }[];
    if (!activeConfigs.some(c => c.reading_mode === 'simplified' && c.lens_intensity === 'balanced')) {
       activeConfigs.push({ reading_mode: 'simplified', lens_intensity: 'balanced' });
    }

    for (const config of activeConfigs) {
      const topArticles = db.prepare('SELECT * FROM articles ORDER BY created_at DESC LIMIT 15').all();
      for (const article of topArticles) {
        await processRawArticleForConfig(article, config.reading_mode, config.lens_intensity);
      }
    }
    
  } catch (err) {
    console.error("RSS Sync Master Error", err);
  } finally {
    isSyncing = false;
  }
}
