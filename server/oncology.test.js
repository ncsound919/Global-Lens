...
test('upsertFinding is idempotent on id', async () => {
  ...
  await upsertFinding(f);
  const list = await getFindingsForPaper('P1');
  assert.equal(list.length, 1);
  assert.equal(list[0].headline, 'Calibrated vs TCGA');
});

test('setFindingOfDay + getFindingOfDay returns today else latest past', async () => {
  await setFindingOfDay('2026-08-20', 'F1');
  const today = await getFindingOfDay('2026-08-20');
  assert.equal(today.finding.id, 'F1');
  const fallback = await getFindingOfDay('2026-08-19');
  assert.equal(fallback.finding.id, 'F1');
});

test('getFindings filters by kind', async () => {
  const kinds = (await getFindings({ kind: 'calibration' })).findings.map((f) => f.id);
