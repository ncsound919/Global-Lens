...
function EmptyState({ label }: { label: string }) {
  return (
    <p className="text-xs text-zinc-600 italic">{label} — the desk is still populating.</p>
  );
}
...
export default function ResearchDeskRail({
  ...
  return (
    <aside className="flex flex-col gap-6">
      <div className="rounded-sm border border-zinc-900 bg-[#0c0c0c]">
        <div className="flex items-center gap-2 border-b border-zinc-900 bg-[#080808] px-5 py-3">
          ...
          <div>
            ...
            {paper ? (
              <button onClick={() => onOpenPaper(paper)} className="group block w-full text-left">
                ...
                <span className="mt-2 inline-flex items-center gap-1 text-[9px] font-bold uppercase tracking-widest text-zinc-500 transition-colors group-hover:text-teal-400">
          ...
          <div className="border-t border-zinc-900 pt-5">
            ...
            {trend ? (
              <button onClick={() => onOpenTrend(trend)} className="group block w-full text-left">
                ...
                <span className="mt-2 inline-flex items-center gap-2 text-[9px] font-bold uppercase tracking-widest text-zinc-500">
                  <TrendingUp className="h-3 w-3" /> {safe(trend.direction || 'flat')} · {formatConfidence(trend.confidence)}
                </span>
              </button>
            ) : (
      ...
      <div className="rounded-sm border border-zinc-900 bg-[#0c0c0c] p-5">
        <span className="mb-2 block text-[9px] font-bold uppercase tracking-[0.2em] text-zinc-500">Editorial Standards</span>
        <p className="text-xs leading-relaxed text-zinc-500">
          Every research item carries an evidence tier (E1–E4) and traces to its source. Findings are measured, never
          fabricated. Original reporting is linked under fair use.
        </p>
        <p className="mt-3 text-[10px] text-zinc-600">{relativeTime(new Date().toISOString())} · Global Edition</p>
      </div>
    </aside>
  );
}
