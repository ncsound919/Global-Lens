      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-zinc-900 bg-zinc-950/90 top-0 sticky z-10 hidden sm:flex">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-xl font-medium text-zinc-100 font-sans tracking-tight">{title}</h2>
          </div>
          <button
            onClick={onClose}
            aria-label={`Close ${title.toLowerCase()}`}
            className="p-2 text-zinc-400 hover:text-white transition-colors bg-zinc-900 hover:bg-zinc-800 rounded-sm"
          >
            <X className="w-5 h-5" />
          </button>
        </div>

        <div className="p-6 overflow-y-auto font-sans text-zinc-300 leading-relaxed space-y-6 text-sm">{children}</div>
...
export const PrivacyPolicyModal: React.FC<LegalModalProps> = ({ onClose }) => (
  <LegalModalShell title="Privacy Policy" icon={Shield} onClose={onClose}>
...
export const TermsOfServiceModal: React.FC<LegalModalProps> = ({ onClose }) => (
  <LegalModalShell title="Terms of Service" icon={FileText} onClose={onClose}>
