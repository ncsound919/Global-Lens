...
export default function FindingOfTheDay({ finding, day }: { finding: FindingProps | null; day: string }) {
  if (!finding) {
    return (
      <section className="rounded-sm border border-zinc-900 bg-ink-900 p-6 text-zinc-500">
  ...
  return (
    <section className="rounded-sm border border-teal-900/60 bg-gradient-to-b from-[#0d1117] to-[#0c0c0c] p-6">
      <div className="flex items-center justify-between gap-3">
        <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-teal-400">Finding of the Day Â· {day}</p>
