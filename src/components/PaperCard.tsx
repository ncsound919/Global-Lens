    ...
    >
      <div className="flex items-center justify-between gap-3 border-b border-zinc-900 bg-[#080808] px-5 py-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {paper.year && (
            <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">{paper.year}</span>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <div className="mt-auto flex items-center justify-between gap-3 border-t border-zinc-900 pt-4">
          <div className="flex flex-col gap-1">
            <span className="text-[9px] font-bold uppercase tracking-widest text-zinc-600">{sourceLabel}</span>
            <span className="text-[10px] uppercase tracking-widest text-zinc-600">{relativeTime(paper.pub_date)}</span>
          </div>
          <div className="flex items-center gap-3">
            {doi && <span className="text-[10px] font-mono text-zinc-600 max-w-[120px] truncate" title={doi}>doi:{doi}</span>}
            {paper.url && (
              <a
                href={paper.url}
                target="_blank"
