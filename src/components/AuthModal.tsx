...
import {
  isEcosystemAuthConfigured,
  linkEcosystemSession,
  signInWithEcosystemGoogle,
} from '../lib/ecosystemAuth';
...
export default function AuthModal({ onClose, onSuccess }: AuthModalProps) {
  ...
  // Shared Supabase auth replaces the old direct-Google popup flow. Hidden
  // until VITE_ECOSYSTEM_SUPABASE_* is configured; local login is unaffected.
  const [ecosystemConfigured] = useState(() => isEcosystemAuthConfigured());
  ...
  const handleGoogleSignIn = async () => {
    ...
    setLoading(true);
    try {
      // Redirects to the shared Supabase project → Google, then back to the
      // app origin. Linking happens in the effect below once the session
      // exists (immediately here if already signed in).
      await signInWithEcosystemGoogle();
      const linked = await linkEcosystemSession();
      if (linked) {
        onSuccess(linked);
      }
      // No linked session yet: the browser is leaving for Google; on return
      // the effect below completes the link.
    } catch (err: any) {
      setError(err.message);
      setLoading(false);
    }
  ...
  React.useEffect(() => {
    // Complete a shared sign-in after the OAuth redirect back to the app:
    // if a Supabase session exists, verify it server-side and link the local
    // user row. No-op when unconfigured (Google button is hidden anyway).
    let cancelled = false;
    linkEcosystemSession()
      .then((linked) => {
        if (!cancelled && linked) onSuccess(linked);
      })
      .catch(() => {
        // Stay on local login; the error surfaces when retrying Google.
      });
    return () => {
      cancelled = true;
    };
