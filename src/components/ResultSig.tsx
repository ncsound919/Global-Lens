...
export default function ResultSig({ label, value }: { label: string; value?: string | null }) {
  ...
  const shown = open ? value : `${value.slice(0, 12)}…`;
  const copy = async () => {
  ...
  return (
    <span className="inline-flex items-center gap-2 rounded-full border border-zinc-800 bg-zinc-950 px-3 py-1 text-[10px] font-mono text-teal-400">
      ...
      <button onClick={(e) => { e.stopPropagation(); setOpen(!open); }} className="text-amber-400 hover:text-amber-300" aria-label={open ? 'Collapse' : 'Expand'}>{open ? '−' : '+'}</button>
      <button onClick={(e) => { e.stopPropagation(); copy(); }} className="text-amber-400 hover:text-amber-300" aria-label="Copy">{copied ? '✓' : '⧉'}</button>
    </span>
  );
}
